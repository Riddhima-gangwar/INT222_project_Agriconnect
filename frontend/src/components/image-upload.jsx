  ...
  const handleFileChange = async (e) => {
    ...
    if (file.size > 10 * 1024 * 1024) {
      toast({
        ...
        description: "Image must be under 10 MB.",
        variant: "destructive",
      });
      return;
    }
    ...
    <div className="space-y-3">
      {value ? (
        <div className="relative group">
          <div
            className={
              containerClass + " border border-border shadow-sm bg-muted"
            }
          >
            <img
              src={value}
          ...
          <div
            className={`absolute inset-0 bg-black/40 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity ${aspectRatio === "square" ? "rounded-full" : "rounded-xl"}`}
          >
            <Button
              type="button"
              size="sm"
          ...
          {uploading ? (
            ...
            <>
              ...
              <p className="text-xs text-muted-foreground">
                Click to browse — JPG, PNG up to 10 MB
              </p>
            </>
          )}
        </div>
