...
import { useListCrops, getListCropsQueryKey, Crop } from "@workspace/api-client-react";
import { Card, CardContent, CardFooter } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { Loader2, Search, MapPin, Calendar, Scale, Sprout } from "lucide-react";
    ...
    <Layout>
      <div className="bg-primary pt-16 pb-24 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 md:px-8 max-w-7xl relative z-10">
          <h1 className="text-4xl md:text-5xl font-bold font-serif tracking-tight text-primary-foreground mb-4">The Marketplace</h1>
          <p className="text-lg md:text-xl text-primary-foreground/80 max-w-2xl font-light">
            Discover verified yields, browse forward contracts, and secure your supply chain directly from the source.
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 md:px-8 py-10 max-w-7xl -mt-16 relative z-20">
        ...
        <div className="bg-card rounded-2xl shadow-xl border border-border p-4 mb-10 flex flex-col md:flex-row gap-4 items-center">
          ...
          <div className="w-full md:w-64 shrink-0">
            <Select value={category} onValueChange={setCategory}>
              <SelectTrigger className="h-14 rounded-xl border-none bg-muted/30" data-testid="select-category">
        ...
        {isLoading ? (
          <div className="flex flex-col justify-center items-center py-32 space-y-4">
            ...
            <p className="text-muted-foreground font-medium">Harvesting data...</p>
          </div>
          ...
          <div className="text-center py-32 bg-muted/20 rounded-3xl border border-dashed border-border/60">
            ...
            <h3 className="text-2xl font-serif font-bold mb-3 text-foreground">No yields found</h3>
            <p className="text-muted-foreground text-lg max-w-md mx-auto">We couldn't find any crops matching your current filters.</p>
            {(search || category !== "all") && (
              <Button 
                variant="outline" 
                size="lg"
      ...
      <Card className="flex flex-col w-full h-full overflow-hidden border-border/40 shadow-sm hover:shadow-xl transition-all duration-500 rounded-3xl group-hover:-translate-y-1 bg-card">
        ...
        <div className="relative aspect-[4/3] bg-muted overflow-hidden">
          ...
          <div className="absolute top-4 left-4 right-4 flex justify-between items-start">
            ...
            {!crop.isAvailable && (
              <Badge variant="destructive" className="shadow-sm">Contracted</Badge>
            )}
          </div>

          {/* Bottom Overlay Content */}
          <div className="absolute bottom-0 left-0 right-0 p-5 text-white transform transition-transform duration-300">
            <h3 className="font-bold font-serif text-2xl leading-tight mb-1 drop-shadow-md line-clamp-2" title={crop.title}>
              {crop.title}
            </h3>
            <p className="text-white/80 text-sm font-medium">by {crop.farmerName}</p>
          </div>
        </div>
        ...
        <CardContent className="p-6 flex-1 flex flex-col bg-background">
          <div className="flex items-end justify-between mb-4 pb-4 border-b border-border/40">
            <div>
              <div className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Asking Price</div>
              <div className="flex items-baseline gap-1">
                <span className="text-2xl font-bold text-primary font-serif">${crop.pricePerUnit.toFixed(2)}</span>
                <span className="text-sm text-muted-foreground">/{crop.unit}</span>
              </div>
            </div>
            <div className="text-right">
              <div className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Volume</div>
              <div className="font-bold text-foreground">{crop.quantity.toLocaleString()} <span className="font-normal text-muted-foreground">{crop.unit}</span></div>
            </div>
          </div>
          ...
          <div className="space-y-3 mt-auto">
            <div className="flex items-center text-sm text-muted-foreground">
              ...
              <span className="truncate">Est. Harvest <strong className="text-foreground ml-1">{format(new Date(crop.harvestDate), 'MMM yyyy')}</strong></span>
            </div>
            <div className="flex items-center text-sm text-muted-foreground">
              <MapPin className="h-4 w-4 mr-3 text-secondary shrink-0" />
              <span className="truncate">{crop.farmerLocation || 'Location unavailable'}</span>
            </div>
          </div>
        </CardContent>
      </Card>
