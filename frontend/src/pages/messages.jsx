...
export default function Messages() {
  ...
  const { data: conversations, isLoading: isConvosLoading } =
    useListConversations(undefined, {
      query: {
      ...
      },
      {
        onSuccess: () => {
          ...
          queryClient.invalidateQueries({
            queryKey: getListMessagesQueryKey({ withUserId: activeUserId }),
          });
          queryClient.invalidateQueries({
            queryKey: getListConversationsQueryKey(),
          });
  ...
  return (
    <Layout>
      <div className="container mx-auto px-4 md:px-8 py-8 max-w-6xl h-[calc(100vh-64px)] flex flex-col">
        <h1 className="text-3xl font-bold font-serif tracking-tight text-foreground mb-6">
          Communications
        </h1>
            ...
            <div className="flex-1 overflow-y-auto">
              {isConvosLoading ? (
                ...
                <div className="divide-y divide-border/40">
                  {filteredConvos.map((convo) => {
                    ...
                    return (
                      ...
                      >
                        <Avatar
                          className={`h-12 w-12 border ${isActive ? "border-primary/20 shadow-sm" : "border-border"}`}
                        >
                          <AvatarFallback
                            className={`${isActive ? "bg-primary text-primary-foreground" : "bg-secondary/20 text-secondary-foreground"} font-serif font-bold text-lg`}
                          >
                        ...
                        <div className="flex-1 min-w-0 overflow-hidden mt-0.5">
                          <div className="flex justify-between items-baseline mb-1.5">
                            <h4
                              className={`font-bold text-base truncate ${isActive ? "text-foreground" : "text-foreground/90"}`}
                            >
                              {convo.userName}
                            </h4>
                            <span
                              className={`text-xs whitespace-nowrap ml-2 ${isActive ? "text-primary font-medium" : "text-muted-foreground"}`}
                            >
                              {format(new Date(convo.lastMessageAt), "MMM d")}
                            </span>
                          </div>
                          <p
                            className={`text-sm truncate ${convo.unreadCount > 0 ? "font-bold text-foreground" : "text-muted-foreground leading-relaxed"}`}
                          >
                            {convo.lastMessage}
                          </p>
                        </div>
                ...
                <div className="text-center p-10 text-muted-foreground flex flex-col items-center justify-center h-full">
                  ...
                  <p className="text-sm">
                    When you contact a farmer or buyer, the conversation will
                    appear here.
                  </p>
                </div>
              )}
            ...
            {activeUserId ? (
              <>
                <div className="px-8 py-5 border-b border-border/60 flex items-center justify-between bg-card z-10 shadow-sm">
                  <div className="flex items-center gap-4">
                    <Avatar className="h-12 w-12 border border-border shadow-sm">
                      <AvatarFallback className="bg-primary text-primary-foreground font-serif font-bold text-lg">
                  ...
                  {isMessagesLoading ? (
                    ...
                    messages.map((msg, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div
                            className={`max-w-[75%] p-4 text-[15px] leading-relaxed shadow-sm ${
                              isMe
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center opacity-50">
                      ...
                      <p className="text-muted-foreground mt-2 max-w-sm">
                        Discuss contract terms, coordinate logistics, or build a
                        new relationship.
                      </p>
                    </div>
                  )}
                ...
                <div className="p-6 bg-card border-t border-border/60 z-10">
                  <form
                    ...
                    className="flex gap-3 bg-muted/30 p-2 rounded-2xl border border-border/60 focus-within:border-primary/50 focus-within:ring-1 focus-within:ring-primary/20 transition-all"
                  >
                    <Input
                    ...
                    <Button
                      type="submit"
                      size="icon"
                      className="h-14 w-14 rounded-xl shrink-0 bg-secondary hover:bg-secondary/90 text-secondary-foreground shadow-sm"
                      disabled={
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-background relative z-10">
                ...
                <p className="text-muted-foreground text-lg max-w-md">
                  Select a conversation from the sidebar to view messages or
                  start a new chat from a contract.
                </p>
              </div>
            )}
          </div>
